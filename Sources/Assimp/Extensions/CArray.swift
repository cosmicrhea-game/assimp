enum CArray<T> {
  @discardableResult
  @_transparent
  static func write<C, O>(_ cArray: inout C, _ body: (UnsafeMutableBufferPointer<T>) throws -> O)
    rethrows -> O
  {
    try withUnsafeMutablePointer(to: &cArray) {
      try body(
        UnsafeMutableBufferPointer<T>(
          start: UnsafeMutableRawPointer($0).assumingMemoryBound(to: T.self),
    ...
    try withUnsafePointer(to: cArray) {
      try body(
        UnsafeBufferPointer<T>(
          start: UnsafeRawPointer($0).assumingMemoryBound(to: T.self),
          count: MemoryLayout<C>.stride / MemoryLayout<T>.stride))
    }
  }
}
