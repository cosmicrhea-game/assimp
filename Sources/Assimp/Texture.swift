...
public class Texture {
  ...
  init(_ texturePtr: UnsafePointer<aiTexture>) {
    self.texturePtr = texturePtr
    let texture = texturePtr.pointee
  ...
  convenience init?(_ aiTexture: UnsafePointer<aiTexture>?) {
    ...
    -> R
  {
    ...
    // Access raw bytes of pcData (aiTexel array is tightly packed BGRA)
    let rawPtr = UnsafeRawPointer(pcData)
    let bytesPtr = rawPtr.assumingMemoryBound(to: UInt8.self)
    return try body(UnsafeBufferPointer(start: bytesPtr, count: count))
  }
}
...
extension Texture: CustomDebugStringConvertible {
  public var debugDescription: String {
    if isCompressed {
      "Texture(compressed \(achFormatHint); \(width.formatted(.byteCount(style: .file))))"
    } else {
      "Texture(\(achFormatHint); \(width) × \(height) px)"
    }
  }
}
