...
final class AssimpTests: XCTestCase {
  ...
  func testFailingInitializer() {
    XCTAssertThrowsError(try Scene(file: "<no useful path>"))
  }
  ...
  func testLoadSceneDAE() throws {
    ...
    XCTAssertEqual(
      scene.materials[0].getMaterialColor(.COLOR_DIFFUSE), SIMD4<Float>(1.0, 1.0, 1.0, 1.0))
    XCTAssertEqual(scene.materials[0].getMaterialString(.TEXTURE(.diffuse, 0)), "./duckCM.tga")
  }

  func testLoadSceneObj() throws {

    let fileURL = try Resource.load(.box_obj)

    let scene: Scene = try Scene(file: fileURL.path)
  ...
  func testLoadScene3DS() throws {
    ...
    XCTAssertEqual(
      scene.materials[0].getMaterialColor(.COLOR_DIFFUSE),
      SIMD4<Float>(0.5882353, 0.5882353, 0.5882353, 1.0))
    XCTAssertEqual(scene.materials[0].getMaterialString(.TEXTURE(.diffuse, 0)), "TEST.PNG")

  }

  func testLoadSceneGLB() throws {
    ...
    XCTAssertEqual(scene.textures[1].width, 1_300_661)
    ...
    XCTAssertEqual(scene.textures[1].textureData.count, 1_300_660)
  ...
  func testLoadAnimatedSceneGLB() throws {
    let fileURL = try Resource.load(.cesiumMan_glb)

    let scene: Scene = try Scene(file: fileURL.path)

    XCTAssertEqual(scene.flags, [])
    XCTAssertGreaterThan(scene.numberOfAnimations, 0)
    XCTAssertGreaterThan(scene.animations.count, 0)

    // At least one mesh should have bones
    let meshesWithBones = scene.meshes.filter { $0.numberOfBones > 0 }
    XCTAssertGreaterThan(meshesWithBones.count, 0)

    // Skeleton sanity: bone names are non-empty and weights sum reasonably per bone
    let firstBones = meshesWithBones.first!.bones
    XCTAssertGreaterThan(firstBones.count, 0)
    XCTAssertNotNil(firstBones.first!.name)

    // Animation channels refer to existing nodes
    let allNodeNames = Set(collectNodeNames(scene.rootNode))
    for anim in scene.animations {
      for ch in anim.channels {
        XCTAssertTrue(allNodeNames.contains(ch.nodeName ?? ""))
      }
    }
  }

  private func collectNodeNames(_ node: Node) -> [String] {
    var names: [String] = []
    if let n = node.name { names.append(n) }
    for c in node.children { names.append(contentsOf: collectNodeNames(c)) }
    return names
  }
}
